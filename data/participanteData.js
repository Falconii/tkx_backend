...
/* CRUD GET */
exports.getParticipante = function (
  id_empresa,
  id_evento,
  id_inscrito,
  inscricao,
...
exports.getParticipantes = function (params) {
  if (params) {
    ...
    if (params.orderby == "")
      orderby =
        "participante.id_empresa,participante.id_evento,participante.id_inscrito,participante.id_inscrito";
    if (params.orderby == "000000")
      orderby =
        "participante.id_empresa,participante.id_evento,participante.id_inscrito,participante.id_inscrito";
    if (params.orderby == "000001")
      orderby =
        "participante.id_empresa,participante.id_evento,participante.id_inscrito,participante.nro_peito";
    ...
    if (params.inscrito_nome.trim() !== "") {
      if (where != "") where += " and ";
      if (params.sharp) {
        where += `inscrito.nome = '${params.inscrito_nome}' `;
      } else {
        where += `inscrito.nome like '%${params.inscrito_nome.trim()}%' `;
      }
    }
    if (params.inscrito_cpf.trim() !== "") {
      if (where != "") where += " and ";
      if (params.sharp) {
        where += `inscrito.cnpj_cpf = '${params.inscrito_cpf}' `;
      } else {
        where += `inscrito.cnpj_cpf like '%${params.inscrito_cpf.trim()}%' `;
      }
    }
    ...
    if (params.contador == "S") {
      ...
      console.log("Select Participantes", strSql);
...
/* CRUD - DELETE */
exports.deleteParticipante = function (
  id_empresa,
  id_evento,
  id_inscrito,
  inscricao,
