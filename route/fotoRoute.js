...
try {
...
catch (err)
	{
		...
		{
			res.status(500).json({ erro: 'BAK-END', tabela: 'UPLOAD-Foto', message: err.message });
		}
	}
})
...
try {
...
catch (err)
	{
		...
		{
			res.status(500).json({ erro: 'BAK-END', tabela: 'GETFOTO-Foto', message: err.message });
		}
	}
})

router.post("/savefotoV2",uploadFoto.single("foto"),async function(req, res) {
try {
	  const { originalname, buffer } = req.file;
	  ...
	  await fotoUploadSrv.saveFotoV2(id_empresa,id_evento,originalname, buffer,id_usuario);
...
router.delete("/deletefoto/:id_empresa/:id_evento/:id",async function(req, res) {
try 
	{
		await fotoUploadSrv.deleteFoto(req.params.id_empresa,req.params.id_evento,req.params.id);
