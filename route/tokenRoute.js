...
/* ROTA GETONE token */
router.get("/:id_empresa/:token/:tipo/:id_usuario", async function(req, res) {
    try {
        const lsLista = await tokenSrv.getToken(
        ...
        if (lsLista == null) {
            res.status(409).json({ message: "Token Não Encontrado." });
        } else {
        ...
        if (err.name == "MyExceptionDB") {
            ...
            res
                .status(500)
                .json({ erro: "BAK-END", tabela: "token", message: err.message });
        }
    }
});
/* ROTA GETALL token */
...
/* ROTA DELETE token */
router.delete(
    "/:id_empresa/:token/:tipo/:id_usuario",
    async function(req, res) {
        try {
            await tokenSrv.deleteToken(
                req.params.id_empresa,
                req.params.token,
                req.params.tipo,
                req.params.id_usuario,
...
router.delete(
    "/deleteTokenByUser/:id_empresa/:id_usuario",
    async function(req, res) {
        try {
            await tokenSrv.deleteTokenByUser(
                req.params.id_empresa,
                req.params.id_usuario,
            );
            res.status(200).json({ message: "Token Excluído Com Sucesso!" });
