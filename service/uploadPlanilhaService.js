  ...
  for await (let linha of dadosPlanilha) {
    ...
    try {
      ...
      const categoria = await categoriaSrv.getCategoriaBySigla(
        id_empresa,
        complementarModel.sigla_categoria,
      );
...
function dados_complementares(campos) {
  ...
  try {
    ...
    if (
      ...
      complementarModel.nro_peito.trim() === "" ||
      isNaN(complementarModel.nro_peito) ||
...
function _inscrito(campos) {
  ...
  try {
    ...
    if (
      inscritoModel.data_nasc == null ||
      inscritoModel.data_nasc.trim() === "" ||
      !shared.isValidDate(inscritoModel.data_nasc)
    ) {
    ...
    if (
      ...
      inscritoModel.sexo.trim() === "" ||
      (inscritoModel.sexo !== "M" && inscritoModel.sexo !== "F")
    ) {
      inscritoModel.sexo = "";
